...
{
    return Matrix4x4<T>(start + 1, start + 2, start + 3, start + 4, start + 5, start + 6, start + 7,
                        start + 8, start + 9, start + 10, start + 11, start + 12, start + 13,
                        start + 14, start + 15, start + 16);
}
...
static void BM_MatrixParamConstruct(benchmark::State& state)
{
    for (auto _ : state)
    {
        Matrix4x4<float> m(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f,
